...
*/
    ...
    function getHistoricalObservedData(obj, bbox) {
        ...
        .then(function (response) {
          ...
          response.data.forEach(function (record) {
            if (undefined !== record[1][obj.options.fips] && String(record[1][obj.options.fips]) !== '-999' && String(record[1][obj.options.fips]) !== '') {
              ...
              data[record[0]] = record[1][obj.options.fips]
            }
          });
          return data;
        });
    ...
    function getHistoricalModelData(obj, bbox) {
      var lastyear = (String(new Date().getFullYear() - 1));
      return $.when.apply($, [
        getLOCAData(obj, bbox, 'loca:wMean:rcp85', '1950', lastyear),
        getLOCAData(obj, bbox, 'loca:allMin:rcp85', '1950', lastyear),
    ...
    function getProjectedModelData(obj, bbox) {
      if (obj.options.frequency === 'annual') {
        var thisyear = (String(new Date().getFullYear()));
        return $.when.apply($, [

          getLOCAData(obj, bbox, 'loca:wMean:rcp45', thisyear, '2099'),
          getLOCAData(obj, bbox, 'loca:allMin:rcp45', thisyear, '2099'),
          getLOCAData(obj, bbox, 'loca:allMax:rcp45', thisyear, '2099'),
          getLOCAData(obj, bbox, 'loca:wMean:rcp85', thisyear, '2099'),
          getLOCAData(obj, bbox, 'loca:allMin:rcp85', thisyear, '2099'),
          getLOCAData(obj, bbox, 'loca:allMax:rcp85', thisyear, '2099')
        ])
        ...
        return $.when.apply($, [
          getLOCAData(obj, bbox, 'loca:wMean:rcp45', '2005', '2099'),
          getLOCAData(obj, bbox, 'loca:allMin:rcp45', '2005', '2099'),
          getLOCAData(obj, bbox, 'loca:allMax:rcp45', '2005', '2099'),
          getLOCAData(obj, bbox, 'loca:wMean:rcp85', '2005', '2099'),
          getLOCAData(obj, bbox, 'loca:allMin:rcp85', '2005', '2099'),
          getLOCAData(obj, bbox, 'loca:allMax:rcp85', '2005', '2099')
        ])
          .then(function (wMean45, min45, max45, wMean85, min85, max85) {
            var data = [];
            [2025, 2025, 2075].forEach(function (yearRange) {
              ['01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12']
                .forEach(function (month) {
                  ...
                  for (var year = yearRange - 15; year < yearRange + 15; year++) {
                    if (wMean45.hasOwnProperty(String(year) + String(month))) {
                      data[yearRange][month]['wMean45'].push(wMean45[String(year) + String(month)]);
                    }
                    if (wMean85.hasOwnProperty(String(year) + String(month))) {
                      data[yearRange][month]['wMean85'].push(wMean85[String(year) + String(month)]);
                    }
                    if (min45.hasOwnProperty(String(year) + String(month))) {
                      data[yearRange][month]['min45'].push(min45[String(year) + String(month)]);
                    }
                    if (min85.hasOwnProperty(String(year) + String(month))) {
                      data[yearRange][month]['min85'].push(min85[String(year) + String(month)]);
                    }
                    if (max45.hasOwnProperty(String(year) + String(month))) {
                      data[yearRange][month]['max45'].push(max45[String(year) + String(month)]);
                    }
                    if (max85.hasOwnProperty(String(year) + String(month))) {
                      data[yearRange][month]['max85'].push(max85[String(year) + String(month)]);
                    }
                  }
                });
            });
            // mean values
            data.forEach(function (yearRange) {
